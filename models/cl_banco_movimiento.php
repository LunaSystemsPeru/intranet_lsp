...
class cl_banco_movimiento
{
    ...
    public  function  verFilasMensual ($periodo) {
        $sql = "select bm.id_movimiento, bm.fecha, bm.descripcion, bm.ingresa, bm.sale, cm.nombre as nom_clasificacion 
        from bancos_movimientos as bm 
        inner join clasificacion_movimientos cm on bm.id_clasificacion = cm.id_clasificacion 
        where concat(year(bm.fecha), lpad(month(bm.fecha), 2, 0)) = '$periodo' and bm.id_banco = '$this->id_banco' 
