...
class cl_venta
{
    ...
    {
        $sql = "select v.periodo_numero, v.id_venta, v.fecha, p.nombre_comercial, ds.abreviatura, v.serie, v.numero, concat(ds.abreviatura, ' | ', lpad(v.serie, 3, 0), ' - ', lpad(v.numero, 5, 0)) as doc_sunat, v.monto_total, (v.monto_total - v.monto_pagado) as monto_deuda, v.estado
        from ventas as v 
    ...
    {
        $sql = "select day(v.fecha) as dia, ifnull(sum(v.monto_total), 0) as total 
        from ventas as v 
    ...
    public function mostrarMes () {
        $sql = "select count(*) as cant_doc, ifnull(sum(v.monto_total),0) as cobrado
        from ventas as v
        where year(curdate()) = year(v.fecha) and month(curdate()) = month(v.fecha)";
        return $this->c_conectar->get_json_rows($sql);
    }
