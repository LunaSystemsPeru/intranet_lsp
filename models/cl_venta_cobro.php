...
class cl_venta_cobro
{
    ...
    public function datosPromediados()
    {
        $sql = "select ifnull(avg(vc.monto),0) as promedio, ifnull(min(vc.monto),0) as minimo, ifnull(max(vc.monto),0) as maximo
        from ventas_cobros as vc
        where year(vc.fecha)  =year(curdate())";
