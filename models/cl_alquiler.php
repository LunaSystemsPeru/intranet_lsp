...
class cl_alquiler
{
    ...
    {
        $sql = "select ifnull(max(id_producto_alquiler) + 1, 1) as codigo 
        from producto_alquiler";
        $this->id_alquiler = $this->c_conectar->get_valor_query($sql, "codigo");
    }
    ...
    {
        $sql = "insert into producto_alquiler values ('" . $this->id_alquiler . "', '" . $this->id_prospecto . "', '" . $this->id_producto_precio . "', '" . $this->id_producto . "', 
        '" . $this->fecha_inicio . "', '" . $this->fecha_pago . "', '" . $this->fecha_siguiente_pago . "', '" . $this->monto_cuota . "', '" . $this->tipo_pago . "')";
    ...
    public function sumaAnual()
    {
        $sql = "select sum(if(year(fecha_pago)=year(curdate()),monto_cuota,0)) as total_pagado,
               sum(if(year(fecha_siguiente_pago)=year(curdate()),monto_cuota,0)) as total_porcobrar
        from producto_alquiler";
        return $this->c_conectar->get_json_rows($sql);
    }

    public function verCobrosMensuales()
    {
        $sql = "select m.id, ifnull(count(pa.id_producto), 0) as contar
        from meses as m
        left join producto_alquiler as pa on  month(pa.fecha_siguiente_pago) = m.id
        GROUP by m.id";
        return $this->c_conectar->get_json_rows($sql);
    }
