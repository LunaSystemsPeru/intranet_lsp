...
class cl_cotizacion
{
    ...
    public function verComparativaMensual () {
        $sql = "select m.nombre,
        sum(c.total) as total_cotizado, 
        count(*) as nro_cotizado, 
        (select ifnull(sum(ct.total), 0) from cotizaciones as ct where ct.estado = 1 and year(ct.fecha) = year(CURRENT_DATE()) and m.id = month(ct.fecha)) as monto_aprobado,
        (select count(*) from cotizaciones as ct where ct.estado = 1 and year(ct.fecha) = year(CURRENT_DATE()) and m.id = month(ct.fecha)) as nro_aprobado
        from cotizaciones as c 
        inner join meses as m on m.id = month(c.fecha)
